...
class Client:
    ...
    # def __init__(self, host="192.168.100.129:8081",
    #              pem_path=os.path.join(os.path.dirname(__file__), "../certs", "121000005l35120456.node1.pem"),
    #              password=None, credit_code="121000005l35120456", cert_name="node1"):
    def __init__(self, host,pem_path, password, credit_code, cert_name):
        self.host = host
    ...
    def __get_sig(self, transaction):
        ...
        sig_bytes = self.__get_pvkey().sign(transaction.SerializeToString(), ec.ECDSA(hashes.SHA256()))
    ...
    def create_trans_deploy(self, trans_id, chaincode_name, chaincode_ver, chaincode_type, \
    ...
    def create_trans_invoke(self, trans_id, chaincode_name, chaincode_ver, func, params):
        trans = peer.Transaction()
        if ("" == trans_id):
            trans.id = str(uuid.uuid4())
        else:
